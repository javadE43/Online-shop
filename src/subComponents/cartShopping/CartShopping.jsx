...
export const CartShopping = () => {
    ...
    const [showproductsshopingcart, setShowproductsshopingcart] = React.useState(false);
    ...
    <>
      ...
      <Box>
          <ClickAwayListener onClickAway={handleClickAway}>
            ...
            onClick={()=>setShowproductsshopingcart(!showproductsshopingcart)}
          >
            <Badge badgeContent={cart.amount} color="error">
               <ShoppingCartIcon />
            </Badge>
          </StyleIconButton>
          <Collapse sx={{position:'absolute',top:'100%',zIndex:'1000'}} in={showproductsshopingcart} timeout="auto" unmountOnExit>
          
          <StylePaper square={false}  elevation={1} component='div'>

            <StyleListCartShoping component="ul" disablePadding>
                  {
                     cart.cartItem && cart.cartItem.map((product,index)=>(
                       <div key={uniqid()}>
                         {/* <Divider /> */}
                      <StylePaperItem square={false}  elevation={1} component='li'>
                      <IconButton  aria-label="delete" onClick={()=>handlerRemoveCart(product)}>
                  ...
                  {cart.cartItem.length>0 ?
                    <>
                      ...
                      <ListItem sx={{display:'flex',justifyContent:'center'}}>
                         <Button onClick={()=>handleraddtocart()} variant="outlined">View All Product</Button>
                      </ListItem>
                    </>
                    :<p>empty cart</p>
                  }
